...
choice_list = sel_string.split('_')
...
# # avg_n_plus_1 = (N)/(N+1)*avg_n + 1/(N+1)*img_n_plus_1
# def update_running_average(img_n_plus_1, N, curr_avg):
# 	# the given image may not yet have an average (i.e., this is the first user who has been assigned this image)
# 	if(N>0):
# 		# avg_n = np.asarray(Image.open(basepath_avg + curr_avg + '.png').convert('L'), dtype=np.float)
# 		avg_n = cv2.imread(basepath_avg + curr_avg + '.png', cv2.IMREAD_GRAYSCALE)
# 		avg_n = np.array(avg_n, dtype=np.float)
# 		avg_n_plus_1 = (N * avg_n + img_n_plus_1) / (N + 1)
# 		# print '<p>{},{}</p>'.format(curr_avg, N)
# 	else:
# 		avg_n_plus_1 =  img_n_plus_1
